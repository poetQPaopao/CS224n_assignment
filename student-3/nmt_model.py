...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # print("[debug]: X shape before CNN:", X.shape)
        X = X.permute(1,2,0)
        X = self.post_embed_cnn(X)
        X = X.permute(2,0,1)

        X_packed = nn.utils.rnn.pack_padded_sequence(X, source_lengths, enforce_sorted=True)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_packed)
        # print("[debug]: ", type(enc_hiddens), type(last_hidden), type(last_cell))
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        # print("[debug]: enc_hiddens shape:", enc_hiddens.shape)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert enc_hiddens_proj.shape[2] == self.hidden_size, "enc_hiddens_proj has incorrect hidden size"
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/generated/torch.squeeze.html
        # print("[debug]: decoder input: " , type(Ybar_t), Ybar_t.shape, type(dec_state), dec_state[0].shape, dec_state[1].shape)
        dec_state = self.decoder(Ybar_t, dec_state)
        dec_hidden, dec_cell = dec_state
        # print("[debug]: dec_hidden shape:", dec_hidden.shape)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)
        # print("[debug]: e_t shape:", e_t.shape)
        ### END YOUR CODE
