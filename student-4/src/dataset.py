...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1) Random truncate between 4 and 7/8 * block_size (inclusive)
        max_trunc = min(len(doc), int(self.block_size * 7 / 8))
        if max_trunc < 4:
            # Document too short; fall back to the whole doc
            trunc_len = len(doc)
        else:
            trunc_len = random.randint(4, max_trunc)
        doc = doc[:trunc_len]

        # 2) Random mask span (~1/4 on average)
        max_mask_len = max(1, trunc_len // 2)  # cap so average is ~1/4
        mask_len = random.randint(1, max_mask_len)
        start = random.randint(0, max(0, trunc_len - mask_len))
        prefix = doc[:start]
        masked_content = doc[start:start + mask_len]
        suffix = doc[start + mask_len:]

        # 3) Build masked string and pad to block_size + 1
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        pad_len = self.block_size + 1 - len(masked_string)
        if pad_len < 0:
            # In rare cases truncation/masking can exceed block budget; trim tail
